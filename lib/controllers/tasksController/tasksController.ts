...
const tasksController = ({ utils, config, constants, models }: any) => {
  const list = async (ctx: any, next: any) => {
    try {
      ...
      const result = await models.tasks.list({
        taskType,
        pageSize,
        currentPage
      });
  ...
  const create = async (ctx: any, next: any) => {
    try {
      ...
      if (task && Object.keys(task).length) {
        ...
        if (www && www.length) {
          const result = await models.tasks.create(www);
  ...
  const getHealth = async (ctx: any, next: any) => {
    const healthcheck = await models.tasks.getHealth();
