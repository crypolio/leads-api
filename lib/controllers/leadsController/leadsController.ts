...
const leadsController = ({ utils, config, constants, models }: any) => {
  const get = async (ctx: any, next: any) => {
    try {
      const { id = "" } = ctx.params;

      if (id && id.length) {
        const result = await models.leads.get(id);
      ...
      ctx.body = {
        reason: "SERVER_ERROR",
        explanation: `Error caught in '/v2/leads' api end point.`
      };
      ctx.status = 501;
    }
  };
  ...
  const getHealth = async (ctx: any, next: any) => {
    const healthcheck = await models.leads.getHealth();
    try {
      ctx.body = {
