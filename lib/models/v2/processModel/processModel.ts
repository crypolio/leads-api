const processModel = (modelOptions: any) => {
	...
	const parseString = (val: string = '') => val.replaceAll('', '').replaceAll('', '');
	...
	const fetchEmails = async (url = '') => {
		try {
			...
			if (emailMatches) {
				emailMatches = emailMatches.filter((email: string[]) => 
			...
			if (taskElts && taskElts.length) {
				for (let t = 0; t < taskElts.length; t += 1) {
					...
					await Promise.all([
						...
						query(
							'DELETE FROM lead_emails WHERE task_id = $2', 
							[ taskId ]
						),
						query(
	...
	const scrappe = async () => {
		...
		try {
			...
			const urls = await page.$$eval('a', (links: string[]) => links.map((link: any) => link.href).filter((href: string) => 
			...
			const scrapePageData = async (url: string) => {
				...
				const name = nameElement ? await newPage.evaluate((element: any) => element.textContent, nameElement) : '';
				...
				let rating = ratingElement ? await newPage.evaluate((element: any) => element.textContent, ratingElement) : '';
				...
				let reviews = reviewsElement ? await newPage.evaluate((element: any) => element.textContent, reviewsElement) : '';
				...
				let category = categoryElement ? await newPage.evaluate((element: any) => element.textContent, categoryElement) : '';

				const addressElement = await newPage.$('button[data-tooltip="Copy address"]');

				let address = addressElement ? await newPage.evaluate((element: any) => element.textContent, addressElement) : '';
				...
				let website = websiteElement ? await newPage.evaluate((element: any) => element.getAttribute('href'), websiteElement) : '';
				// website = `${website}`;

				const phoneElement = await newPage.$('button[data-tooltip="Copy phone number"]');

				let phone = phoneElement ? await newPage.evaluate((element: any) => element.textContent, phoneElement) : '';
			...
			for (let i = 0; i < urls.length; i += batchSize) {
				...
				const batchResults = await Promise.all(batchUrls.map((url: string) => scrapePageData(url)));
			...
			await Promise.all([
				...
				query(
					'DELETE FROM lead_emails WHERE task_id = $1', 
					[ taskId ]
				),
				query(
